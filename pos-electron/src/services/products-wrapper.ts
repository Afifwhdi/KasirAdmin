...
import { productsApi, ProductsPaginationParams, ProductsPaginationResponse } from '@/features/products/services/api';
  ...
  async getAll(params?: ProductsPaginationParams): Promise<ProductsPaginationResponse> {
    if (isElectron()) {
      ...
      if (params?.category_id) {
        filtered = filtered.filter((p: any) => p.category === params.category_id.toString());
      }
