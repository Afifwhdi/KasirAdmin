...
export const transactionsApi = {
  ...
  ): Promise<TransactionsPaginationResponse> {
  ...
  async updateStatus(id: number, status: 'pending' | 'paid' | 'cancelled' | 'refunded') {
    const res = await fetch(
      `${API_CONFIG.BASE_URL}${API_ENDPOINTS.TRANSACTIONS}/${id}/status`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async payBon(id: number, cashReceived: number, changeAmount: number) {
    const res = await fetch(
      `${API_CONFIG.BASE_URL}${API_ENDPOINTS.TRANSACTIONS}/${id}/pay`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async getDetail(id: number) {
    const res = await fetch(
      `${API_CONFIG.BASE_URL}${API_ENDPOINTS.TRANSACTIONS}/${id}`
    );
