...
const TransactionsPage = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; className: string }> = {
      paid: { label: "Lunas", className: "bg-green-500 hover:bg-green-600 text-white" },
      Lunas: { label: "Lunas", className: "bg-green-500 hover:bg-green-600 text-white" },
      pending: { label: "Menunggu", className: "bg-yellow-500 hover:bg-yellow-600 text-white" },
      Menunggu: { label: "Menunggu", className: "bg-yellow-500 hover:bg-yellow-600 text-white" },
      cancelled: { label: "Dibatalkan", className: "bg-red-500 hover:bg-red-600 text-white" },
      Dibatalkan: { label: "Dibatalkan", className: "bg-red-500 hover:bg-red-600 text-white" },
      refunded: { label: "Refund", className: "bg-blue-500 hover:bg-blue-600 text-white" },
  ...
  const canCancel = (status: string) =>
    status === "paid" || status === "Lunas" || status === "pending" || status === "Menunggu";
  const canRefund = (status: string) => status === "paid" || status === "Lunas";
  const canPay = (status: string) => status === "pending" || status === "Menunggu";
              ...
              {isLoading ? (
                ...
                transactions.map((transaction) => (
                  <TableRow key={transaction.id}>
                    ...
                    <TableCell>{transaction.nama_customer || "Umum"}</TableCell>
                    ...
                    <TableCell>
                      {transaction.transaksi_dibuat
                        ? format(new Date(transaction.transaksi_dibuat), "dd MMM yyyy, HH:mm", {
                            locale: id,
                          })
                        : "-"}
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center justify-center gap-2 flex-wrap">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleViewDetail(transaction.id, transaction.transaction_number)
                          }
                          className="h-8"
                        >
          ...
          <div className="flex-shrink-0 border-t p-4">
            <div className="flex items-center justify-between">
              <div className="text-sm text-muted-foreground">
                ...
                {Math.min(meta.page * meta.limit, meta.total)} dari {meta.total} transaksi
              </div>
